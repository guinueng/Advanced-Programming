...
vertical_split::vertical_split(vertical_split const& other) : two_widget_container(other.first, other.second) {}
...
char vertical_split::char_at(uint x, uint y, uint w, uint h) const{
    if(y < h / 2)
        return this -> first -> char_at(x, y, w, h);
    else
        return this -> second -> char_at(x, y - h / 2, w, h);
}
