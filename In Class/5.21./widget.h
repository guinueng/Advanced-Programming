...
class vertical_split : public two_widget_container{
    ...
    char char_at(uint, uint, uint, uint) const override;
    widget* copy() const override { return new vertical_split(this -> first, this -> second); };
};

class overlapping : public two_widget_container{
    public:
    overlapping(widget* back, widget* front) : two_widget_container(back, front) {}
    overlapping(overlapping const& other) : two_widget_container(other.first, other.second) {}
    char char_at(uint, uint, uint, uint) const override;
    widget* copy() const override { return new overlapping(this -> first, this -> second); }
};
